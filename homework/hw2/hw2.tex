...
\item Aaron Gonzales:
	\begin{itemize}
		\item Ex. 7: I believe i can follow your example, though your
			explanation of how the accepting state connects to the start state
			is slightly unclear. There are minor typesetting issues that could
			be corrected.
		\item Ex. 8: Note, in the \\align or \\equation environments, you can
			use the command \\text{xxx} to display text in a more readable
			format - i'd suggest making use of this in the future for things
			like ``a is a string from A''
		\item Ex. 11: not attempted
		\item Ex. 10: I don't know if i follow your answer at all. You seem to
			be arbitrarily claiming that you cna make an odd-numbered state DFA
			and stating that $S^{Yes}_2 \in S^{Yes}_2$ which is tautological.
		\item Ex. 13,14,15: Not attempted
	\end{itemize}
	\end{itemize}
	\begin{itemize}
		\item Manasa Navada
			\begin{itemize}
				\item Ex. 7: Explanation seems correct
				\item Ex. 8: There are some typing errors which need to be
					fixed. I could not follow the explanation clearly.
				\item Ex. 10: Could not really understand the solution.
				\item Ex. 11: Incomplete solution
				\item Ex. 13: Correct
				\item Ex. 14: correct
				\item Ex. 20: Explanation is correct but I think we should use
					Aaron’s solution for this exercise since it is more
					detailed.
			\end{itemize}
	\end{itemize}
